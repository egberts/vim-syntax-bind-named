...
This project aims to replace the stock Vim syntax hilighting for ISC Bind
...
nearly all the revised psuedo-BNF syntaxes for every `named.conf` statement
keywords encountered.  These psuedo-BNF syntaxes may (often) not match the 
...
* replace the bindzone.vim, but that is something that I 
...
* make this work steadily across ALL versions of Bind, but in
  starting with the current v9.15 and then OUTWARD toward v4 and 
...
at [egberts' repo at Github](https://github.com/egberts/bind_parser/bind_parser/examples/rough-drafts/namedconfglobal.py). 
...
* keep narrowing down the offending line until it stopped offending, hopefully
  it is just to just one (or few) lines.  No need for your entire named.conf
  Don't forget to change all IP addresses (unless that breaks too) for
  your privacy sake.
 
* post/file the offending line over at my [Github issue](https://github.com/egberts/vim-syntax-bind-named).
...
see my DEBUG.md on how to debug a Vim syntax file. 
...
of 9 groupings of parenthesises, even if I used the "\%( ... \)" 
notation (instead of "\( ...\)").  
